...
const getExperiences = async (req, res) => {
    try {
        ...
        "SELECT id, empresa, puesto, DATE_FORMAT(fecha_inicio, '%Y-%m-%d') AS fecha_inicio, DATE_FORMAT(fecha_fin, '%Y-%m-%d') AS fecha_fin, responsabilidades, salario FROM experiences"
        );
        res.json(result);
    } catch (error) {
        res.status(500);
